...
const CONFIG_PATH = path.join(__dirname, 'src', 'ajv-config.js');
...
(async () => {
    try {
        await fs.writeFile(CONFIG_PATH, FINAL_AJV_CONFIG_CODE.trim());
        console.log(`\n✅ SUCCESS: src/ajv-config.js fixed and updated with ajv-formats registration.`);
    } catch (error) {
        console.error(`\n❌ ERROR: Failed to write src/ajv-config.js. Details: ${error.message}`);
    }
})();
